 ...
 */
import { type CompressResult, type ImageLimits } from './compressor.js';
/** 防重入标记：注入事件对象上挂本插件私有符号。 */
export declare const INJECTION_MARK: unique symbol;
 ...
 */
export declare function shouldTakeOver(facts: TakeOverFacts): boolean;
/** 运行时投影防御：非负数值检查失败即视为无投影（AC-11 放行）。 */
export declare function asImageLimits(value: unknown): ImageLimits | undefined;
/** 一批次单张结果：携带原始顺序下标。 */
 ...
 */
export declare function processBatch(files: readonly File[], limits: ImageLimits, compress: (file: File, limits: ImageLimits) => Promise<CompressResult>): Promise<ProcessedBatch>;
/** 事件对象上防重入标记读取。 */
export declare function isMarked(event: Event, mark?: symbol): boolean;
/** 读取 drop 事件的拖放文件（非图片拖放同样读取，判定决定是否接管）。 */
export declare function filesOfDrop(event: Event): File[];
/** 读取 paste 事件的剪贴板文件（React 合成事件由捕获阻断，这里读原生 ClipboardEvent）。 */
export declare function filesOfPaste(event: Event): File[];
/**
 * 注入：按原始顺序构造 DataTransfer 逐项 add，派发 document 冒泡 drop 事件，
 * 事件对象挂防重入标记。返回是否成功派发。
 */
export declare function injectDrop(files: readonly File[], mark?: symbol): boolean;
 ...
 */
export declare function attachIntakeListeners(deps: IntakeDeps, mark?: symbol): () => void;
