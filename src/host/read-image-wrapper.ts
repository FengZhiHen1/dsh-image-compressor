...
import type { CompressibleMediaType } from './engine.js'
import { compressHostImage, hostOutputName, type CompressedImage, type HostImageLimits } from './engine.js'
...
function sessionCwdOf(exec: ToolDispatchExecution, requestedPath: string): string | undefined {
...
function isCompressibleFailure(result: ToolExecutionResult): boolean {
  if (!result.isError) return false
  if (result.error.info?.code === 'FS_TOO_LARGE') return true
  return result.error.message.includes(RETRYABLE_REFUSAL_SUFFIX)
}

/** 图像能力门（官方 assertImageCapableRoute 的镜像）：仅在跳过官方路径前调用。 */
async function imageRouteCapable(ctx: Context, exec: ToolDispatchExecution): Promise<boolean> {
  try {
...
export function createReadImageCompressor(
  ...
  async function compressAndRespond(
    ...
    const note =
      `dsh-image-compressor auto-compressed this image to fit deployment limits: ` +
  ...
  return async function readImageCompressor(exec, next) {
    ...
    const args = exec.arguments as { readonly file_path?: unknown } | undefined
    const filePath = typeof args?.file_path === 'string' ? args.file_path : undefined
    if (filePath === undefined || filePath.trim().length === 0) return next()

    const declared = IMAGE_EXTENSIONS[extname(filePath).toLowerCase()]
    if (declared === undefined) return next() // 非图片扩展名：官方报错
    if (declared === 'image/gif') return next() // GIF 动画不压缩（与客户端 R-07 一致）

    const attachments = ctx.get('attachments')
    if (attachments === undefined) return next()
    const limits = attachments.imageLimits
    if (!limits.mediaTypes.includes(declared as 'image/png' | 'image/jpeg' | 'image/webp')) return next()
    ...
    const effectiveLimits: HostImageLimits = {
    ...
    if (info.size !== undefined && info.size > effectiveLimits.maxImageBytes) {
      ...
      if (info.size > HOST_READ_CEILING) return next()
      if (!(await imageRouteCapable(ctx, exec))) return next() // 跳过官方前自核能力门
      return (await compressAndRespond(exec, target, info.version, declared as CompressibleMediaType, effectiveLimits)) ?? next()
    }

    // 字节未超限（或后端不报 size）：官方先行，零介入；仅修复可压缩拒绝。
    const result = await next()
    if (!isCompressibleFailure(result)) return result
    return (await compressAndRespond(exec, target, info.version, declared as CompressibleMediaType, effectiveLimits)) ?? result
  }
}
